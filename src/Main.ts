...
function addRequestToEmailsTab(emailsSheet: GoogleAppsScript.Spreadsheet.Sheet, currentCityRequest: string[]): number {
    ...
    try {
        if (Utils.arrayContainsValue(existingRequestIdsQueuedForSendingEmail, 0, idToReturn)) {
            return 0;
        }
    } catch (e: unknown) {
        console.error("Utils.arrayContainsValue :: failure ", e);
        console.exception("existing request check failure ", e);
    }
    ...
    cityRequestIdMap.forEach((setOfRequestIdsForCity: Set<number>, currentCity: string) => {
        ...
        setOfRequestIdsForCity.forEach((requestId: number) => {
            ...
            if (rowNumber !== -1) {
                ...
                let cityResponseHolder: CityResponseHolder = new CityResponseHolder(requestStatus, requestContacted, requestTentativeClosingDate);
